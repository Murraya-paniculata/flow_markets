...
- **FlowMarkets 交易研究**: `POST /api/v1/flow-markets/analyze`、`POST /api/v1/flow-markets/analyze/stream`；Crew 模式 `technical_only`（默认）或 `full`。`full`：两段链 + 6.3 synthesis（治理后 technical + stats）+ 6.4 trader/portfolio（`state_machine_summary` / `signal_quality` / `execution_stats_context`，**无回测引擎**）；上游工具见 `get_market_ticker_summary`、`baidu_search`。
