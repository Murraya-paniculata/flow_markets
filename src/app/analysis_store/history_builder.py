"""组装 get_chan_structure 的 history 块（Phase 2.4 system_stats）。"""
...
def compute_state_machine_hints(
    ...
    if hit_rate < MIN_WIN_RATE_OBSERVE_ONLY:
        hints["recommended_floor"] = "OBSERVE_ONLY"
        hints["message"] = f"历史胜率 {hit_rate*100:.1f}% 偏低，建议 OBSERVE_ONLY"
    elif hit_rate < MIN_WIN_RATE_WAIT_CONFIRMATION:
        hints["recommended_floor"] = "WAIT_CONFIRMATION"
        hints["message"] = f"历史胜率 {hit_rate*100:.1f}% 一般，至少 WAIT_CONFIRMATION"
    else:
...
def build_history_block(snapshot: ChanStructureSnapshot) -> dict[str, Any]:
    """构建工具 JSON 的 history 段（2.4：system_stats + state_machine_hints）。"""
    ...
    try:
        ...
        hints = compute_state_machine_hints(stats, symbol, interval)
        available = system_stats.get("has_data", False)
        return {
            "available": available,
            ...
            "similar_cases": {"has_data": False, "message": "Phase 2.5"},
